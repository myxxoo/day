                ...
                <div class="past_content">
                    ...
                    <div class="thumb"><!-- 缩略图 -->
                        <?php if ( has_post_thumbnail() ) { ?>
                            <a target="_blank" href="<?php the_permalink() ?>"><?php the_post_thumbnail('gallery150cc200', array( 'alt' => trim(strip_tags( $post->post_title )), 'title' => trim(strip_tags( $post->post_title )))); ?></a>
                        <?php } else { ?>
                            <a target="_blank" href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title(); ?>"><img src="<?php echo get_featcat_image(); ?>"  alt="<?php the_title(); ?>" /></a>
                        <?php } ?>
                    </div>
                    <?php if (has_excerpt()) { ?> <?php the_excerpt() ?> <?php } else{
                        echo mb_strimwidth(strip_tags(apply_filters('the_content', $post->post_content)), 0, 520,"......"); } ?>
